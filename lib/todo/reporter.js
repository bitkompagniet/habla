...
function reportGrouping(issues, group, headline, headlineColor, rowColor, slice) {
	if (issues[group]) {
		...
		if (slice) console.log(rowColor(`Currently limiting issues to ${slice}. Use flag --all to show all issues`))
		console.log()
		console.log()
	}
}

module.exports = (sortedIssues) => {
	...
	if (sortedIssues.noDeadline && _.keys(sortedIssues).length === 1) { // if the there is not other groupings than noDeadline, then it should niether be grey font or limited.
		...
		if (true) {
			reportGrouping(sortedIssues, 'noDeadline', 'No deadline:', chalk.grey, chalk.grey, 10);
		}
	}
};
