...
module.exports = function (sortedIssues) {
	...
	if (sortedIssues.past.length) {
		console.log('');
		console.log(chalk.red.bold('Past deadline:'));
		sortedIssues.past = _.sortBy(sortedIssues.past, (issue) => {
	...
	let color;
	if (!sortedIssues.past.length && !sortedIssues.today.length) {
		color = chalk.white;
	...
	sortedIssues.otherIssues = _.sortBy(sortedIssues.otherIssues, (issue) => {
		if (issue.deadline != null) return moment(issue.deadline).dayOfYear();
		return 1000;
	});
