...
module.exports = {
	...
	thisRepo: {
		me: async function(tokens) {
			...
			const projectSearch = await gitlab(tokens).get(`projects/search/${currentProject}`);
			...
			const user = await gitlab(tokens).get(`user`);
			...
			issues.data = _.filter(issues.data, (issue) => {
				return issue.assignee ? issue.assignee.id === user.data.id : false; // Should be me. Not just somebody.
			});
			return issues.data
		},
