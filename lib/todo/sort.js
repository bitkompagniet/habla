...
function sortGitlab(issues) {
	return _.map(issues, (issue) => {
		...
		return createDataStructure(issue.title, issue.description, issue.milestone, dateOrNull(dueDate), issue.labels, issue.projectName, issue.web_url, 'GitLab');
	});
}
...
function sortTrello(issues) {
	return _.map(issues, (issue) => {
		...
		return createDataStructure(issue.name, issue.desc, null, dateOrNull(dueDate), issue.labels, issue.boardName, issue.shortUrl, 'Trello');
	});
}
