...
function sortGithub(issues) {
	return _.map(issues, (issue) => {
		...
		return createDataStructure(issue.title, issue.body, issue.milestone, dateOrNull(dueDate), createLabel(issue.labels), (issue.repository) ? issue.repository.name : 'undefined', issue.html_url, 'GitHub');
	});
}
