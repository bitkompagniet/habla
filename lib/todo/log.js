...
function format(title, labels, project, link, deadline) {
	const b = chalk.bold;
	const possibleDeadline = (deadline) ? moment(deadline).format('Do MMM YY') + ' ' : '';
	...
	// return ([b(cutString(title, 40)) + ' ' + _.join(labelColor(labels), ' '), chalk.underline(project), possibleDeadline, chalk.grey(link)]);
}
...
function logGrouping(issues, group, headline, headlineColor, rowColor, slice) {
	if (issues[group]) {
		...
		_.forEach(_.slice(issues[group], 0, slice), (issue) => {
		...
		if (slice) console.log(rowColor(`Currently limiting issues to ${slice}. Use flag --all to show all issue`))
		console.log()
		console.log()
	}
}
...
module.exports = function (sortedIssues) {
	logGrouping(sortedIssues, 'past', 'Past deadline:', chalk.red, chalk.white);
	logGrouping(sortedIssues, 'today', 'Due today:', chalk.green, chalk.white);
	logGrouping(sortedIssues, 'tomorrow', 'Due tomorrow:', chalk.green, chalk.white);
	logGrouping(sortedIssues, 'thisWeek', 'Due this week:', chalk.green, chalk.white);
	logGrouping(sortedIssues, 'nextWeek', 'Due next week:', chalk.white, chalk.white);
	logGrouping(sortedIssues, 'afterNextWeek', 'Due after next week:', chalk.grey, chalk.white);
	if (sortedIssues.noDeadline && _.keys(sortedIssues).length === 1) { // if the there is not other groupings than noDeadline, then it should niether be grey font or limited.
		logGrouping(sortedIssues, 'noDeadline', 'No deadline:', chalk.green, chalk.white);
	} else {
		if (true) {
			logGrouping(sortedIssues, 'noDeadline', 'No deadline:', chalk.grey, chalk.grey, 10);
		}
	}
};
