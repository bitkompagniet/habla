...
module.exports = {
	...
	thisRepo: {
		me: async function(tokens) {
			...
			const issues = await gitlab(tokens).get(`projects/${currentProject}/issues`, { per_page: 100 });
		...
		unassigned: async function(tokens) {
			return await gitlab(tokens).get(`/projects/${project.id}/issues`).then((issuesInProject) => {
				return issuesInProject.filter(() => {
